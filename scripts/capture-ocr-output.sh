...
Required environment:
  LLM credentials must be configured for OCR to generate reviews.
  See OCR documentation for supported LLM provider configuration.
...
if ! "$OCR_BIN" review --from main --to HEAD --format json --audience agent > "$WORK_DIR/ocr-output.json" 2>"$WORK_DIR/ocr-stderr.txt"; then
  ...
  echo "" >&2
  echo "LLM credentials must be configured for OCR to generate reviews." >&2
  echo "See OCR documentation for supported LLM provider configuration." >&2
  exit 1
fi

# Check output is valid JSON
if ! python3 -c "import json; json.load(open('$WORK_DIR/ocr-output.json'))" 2>/dev/null; then
  echo "Warning: OCR output is not valid JSON, saving as-is" >&2
fi

# Copy to output path
mkdir -p "$(dirname "$OUTPUT_PATH")"
cp "$WORK_DIR/ocr-output.json" "$OUTPUT_PATH"
